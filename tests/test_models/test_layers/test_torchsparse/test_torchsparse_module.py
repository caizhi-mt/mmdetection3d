...
def test_TorchsparseConvModule():
    ...
    coordinates = torch.tensor(
        [[12, 819, 131, 0], [16, 750, 136, 0], [16, 705, 232, 1],
         [35, 930, 469, 1]],
        dtype=torch.int32).musa()  # n, 4(ind_x, ind_y, ind_z, batch)

    # test
    input_sp_tensor = SparseTensor(voxel_features, coordinates)

    self = TorchSparseConvModule(4, 4, kernel_size=2, stride=2).musa()
...
def test_TorchsparseResidualBlock():
    ...
    sparse_block0 = TorchSparseBasicBlock(4, 16, kernel_size=3).musa()
    sparse_block1 = TorchSparseBottleneck(4, 16, kernel_size=3).musa()
